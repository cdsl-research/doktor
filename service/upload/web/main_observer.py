...
# ConcreteObserver1
class DBUpdate(Observer):
    def __init__(self):
        pass

    def update(self, model):
        # databaseの更新
        fs.put(model.payload)
...
class Logging(Observer):
    ...
    def update(self, model):
        ...
        # ログに書き込み
        log = model.payload
        with open('doktor-log.txt', 'a') as f:
            print(now + "payload: " + log, file=f)

# 将来的にはログ専用サーバーに同じ内容を登録したい
# class CloudLogging(Observer):
#     def __init__(self):
#         pass
#
#     def update(self, model):
...
#         # ログに書き込み
#         log = model.payload
#         logdb = mongodb://doktor-log:27017/
#         logfs = gridfs.GridFS(logdb)
#         logfs.put(model.payload)

obs = ConcreteModel()
obs.add_observer(DBUpdate())
obs.add_observer(Logging())
